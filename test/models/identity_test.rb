...
class IdentityTest < ActiveSupport::TestCase
  	...
  	test 'Valid Identity' do
  		user = users(:one)
  		identity = Identity.new(user_id: user.id, provider: "ProviderTest", uid: "TestUID")
  	...
  	test 'Invalid without user' do
  		...
  		assert_not_equal identity.errors.count, 0, 'Identity is valid without an User'
  	...
  	test 'Invalid if incomplete OAuth info without identity' do
  		...
  		assert_not_equal identity.errors.count, 0, 'Identity is valid without a Provider'
  	...
  	test 'Invalid if incomplete OAuth info without uid' do
  		...
  		assert_not_equal identity.errors.count, 0, 'Identity is valid without an UID'
  	...
  	test 'Valid if not OAuth info without uid and identity' do
  		...
  		assert_equal identity.errors.count, 0, 'Identity is not valid without an UID and an Identity'
