class MeetupsController < ApplicationController
  ...
  def index
    ...
    @most_recent = Meetup.where('date < ?', Date.today).order('date DESC').first(3)
    @most_popular = Meetup.where('date < ?', Date.today).left_joins(:assistances).group(:id).order('AVG(assistances.mark) DESC').first(3)
  end
